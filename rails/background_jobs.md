...
[Honeybadger insights](https://docs.honeybadger.io/guides/insights/) are a feature that can be leveraged for visualizing failures trends in your jobs.
...
There are two ways to specify the [queues configuration](https://github.com/bensheldon/good_job?tab=readme-ov-file#optimize-queues-threads-and-processes) for Good Job.
...
```
ENV['GOOD_JOB_QUEUES'] = mailers:1;default:3;pdf:1
```
...
It is often useful to update the user interface regarding the status of a background job. If your application uses Hotwire, a handy way to do this is to leverage [Turbo::Boradcastable](https://rubydoc.info/github/hotwired/turbo-rails/Turbo/Broadcastable) in an after_perform hook:
